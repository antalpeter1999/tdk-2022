...
def main():
    ...
    load_init = [[0.0, 0, 0.8], [-0.6, 0.6, 0.79], [-0.3, -0.6, 0.9]]
    load_target = [[2.2, 1.0, 0.78], [1.8, 1.0, 0.78], [1.4, 1.0, 0.83]]
    init_pos = [-0.5, 2.0, 1.8, np.pi/2]#, [load_target[0] - [0.3, 0, 0]]]

    pos_ref, vel_ref, yaw_ref, ctrl_type = None, None, None, None
    for num_sec in range(len(load_init)):
        ...
        pos_ref_, vel_ref_, yaw_ref_, ctrl_type_ = construct(init_pos_rel, load_target_rel, plot_result=False)
    ...
    cam.lookat, cam.distance = [1.9, -1.5, 2], 1.5
