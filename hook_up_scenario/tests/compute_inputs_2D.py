...
if __name__ == "__main__":
    ...
    rL = sp.Matrix([make_traj(x_c, t), make_traj(z_c, t)])
    ...
    plt.plot(np.array(tau_arr).astype(np.float64))
    # plt.plot(np.array(rL_arr).astype(np.float64)[:, :, 0])
    # plt.plot(np.array(phi_arr).astype(np.float64))
    plt.show()
    # print(F_arr)
    # print(tau_arr)
    #
    # r0 = np.array([r.subs(t, 0)[0], 0, r.subs(t, 0)[1]])
    # dr0 = np.array([sp.diff(r, t).subs(t, 0)[0], 0, sp.diff(r, t).subs(t, 0)[1]])
    # phi0 = phi.subs(t, 0)
    # phi0 = np.array(phi0, dtype=float)
    # R0 = np.array([[np.cos(phi0), 0, np.sin(phi0)], [0, 1, 0],
    #                [-np.sin(phi0), 0, np.cos(phi0)]])
    # om0 = np.array([0, om.subs(t, 0), 0])
    #
